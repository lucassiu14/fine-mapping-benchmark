...
#
# A COPY of the iter004_* analysis script of the same name, changed only where
# Iteration 005 stratifies differently. Iteration 004 is the projects standard
# and is deliberately NOT touched: every iter004_* file stays byte-identical to
# what produced the standing results, so nothing exploratory can perturb them.
#
# The one substantive difference is the stratification. Iteration 004 splits on
# (model, annotation_type); Iteration 005 holds model constant at "sparse" and
# varies the annotation -> causality RELATIONSHIP, so those keys would collapse
# its twelve rows into two cells and average additive, cooccur, nonmono, mixed
# and threshold together - the five arms whose difference IS the question.
# enrichment_fold does not rescue it: it is 5.4 on every non-null arm.
#
# See docs/autoresearch/iteration-005-REVERT.md.
# =============================================================================
...
#' The strata actually present in a collected table.
#'
#' Iteration 004 stratifies on (model, annotation_type): six cells. Iteration
#' 005 holds model constant at "sparse" and varies the annotation -> causality
#' RELATIONSHIP instead, so those same six-cell definitions would collapse all
#' twelve of its rows into two strata and average additive, cooccur, nonmono,
#' mixed and threshold into a single number - the five arms whose difference is
#' the entire question. `enrichment_fold` does not save it either: it is 5.4 on
#' every non-null arm, so only the null arm would stay distinguishable.
#'
#' Detection is on the DATA, not on a flag, so Iteration 004 tables (which have
#' no `relationship` column) are returned unchanged and every existing analysis
#' is bit-identical.
strata_for <- function(d) {
  if (is.null(d$relationship)) return(STRATA)
  rel <- sort(unique(as.character(d$relationship[!is.na(d$relationship)])))
  if (!length(rel)) return(STRATA)
  ann <- sort(unique(as.character(d$annotation_type[!is.na(d$annotation_type)])))
  out <- list()
  for (r in rel) for (a in ann)
    out[[length(out) + 1L]] <- list(
      key    = sprintf("%s_%s", r, a),
      filter = list(relationship = r, annotation_type = a))
  out
}
...
stratum_subset <- function(d, stratum) {
  # A stratum is defined by a set of field = value constraints. Iteration 004's
  # six strata constrain (model, annotation_type); Iteration 005's twelve
  # constrain (relationship, annotation_type). Reading the constraints off the
  # stratum rather than hardcoding two column names keeps every downstream
  # sense script identical between the two.
  filt <- stratum$filter
  if (is.null(filt)) filt <- list(model = stratum$model,
                                  annotation_type = stratum$annot)
  keep <- rep(TRUE, nrow(d))
  for (nm in names(filt)) {
    # A constraint on a column the table does not carry cannot be honoured. Do
    # NOT silently ignore it - that would return a SUPERSET of the stratum and
    # pool exactly what the stratification exists to keep apart.
    if (is.null(d[[nm]])) return(NULL)
    keep <- keep & !is.na(d[[nm]]) & as.character(d[[nm]]) == filt[[nm]]
  }
  s <- d[keep, , drop = FALSE]
  if (!nrow(s)) return(NULL)
  free <- character(0)
  for (v in c("pc", "enrich", "S", "phi", "region_size")) {
    # A column the table does not carry cannot be a free factor. Guarding here
    # rather than assuming all five are present keeps the function usable on the
    # narrower tables the sense scripts build (and on test frames) instead of
    # failing inside droplevels() with an unrelated-looking error.
    if (is.null(s[[v]])) next
