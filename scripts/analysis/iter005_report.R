...
out_dir  <- args[3] %||% "results/iter005"
...
keep <- c("job_dir", "model", "p_causal", "annotation_type", "enrichment_fold",
          ...
          # `relationship` MUST survive this join. It is the stratification
          # variable for Iteration 005; dropped here, every downstream script
          # sees twelve rows that differ only by job_dir and pools the five
          # non-null relationships into one cell. intersect() below makes both
          # names harmless on an Iteration 004 grid, which has neither.
          "relationship", "n_informative")
...
# >>> ITERATION 005 ONLY - TEMPORARY (see iteration-005-REVERT.md) <<<
# Carry `relationship` through the aggregation so L3 can be stratified on it.
# This does NOT change the grouping: job_dir is already in DESIGN and uniquely
# determines relationship, so the cells are identical either way - the column
# would otherwise simply be dropped by aggregate().
if ("relationship" %in% names(L1)) DESIGN <- c(DESIGN, "relationship")
# >>> END ITERATION 005 TEMPORARY BLOCK <<<
...
# ---------------------------------------------------------------------------
for (script in c("iter005_sense_v.R", "iter005_sense_d.R", "iter005_sense_g.R")) {
  message("\n>>> ", script)
