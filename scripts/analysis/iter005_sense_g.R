...
collect_dir <- args[1] %||% "results/iter005"
out_dir     <- if (length(args) >= 2) args[2] else "results/iter005/sense_g"
...
wide_by_method <- function(d, response) {
  ...
  meta_cols <- c(".cell", "job_dir", "S", "phi", "region_size",
                 "pc", "enrich", "model", "annotation_type")
  # ITER-005 (temp): the stratification column must survive into the wide
  # table, or the re-filter at the kappa-decision stage below cannot honour it.
  if ("relationship" %in% names(d)) meta_cols <- c(meta_cols, "relationship")
  meta <- unique(d[, meta_cols])
  merge(w, meta, by = ".cell", all.x = TRUE)
}

all_kappa <- list(); all_delta <- list(); all_excl <- list()

for (st in strata_for(reps)) {
