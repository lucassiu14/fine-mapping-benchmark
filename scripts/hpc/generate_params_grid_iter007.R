...
#
# DESIGN (user-specified 2026-09-14). 10 rows:
#   LD           in-sample; reference panels of 500, 750, 1500, 2000       5
#   model        sparse only (the user dropped sparse_inf)                 1
...
MODEL            <- "sparse"          # p_causal does not apply to the sparse model
...
for (at in ANNOTATION_TYPES) {
  for (nr in N_REF_LEVELS) {
    ...
    rows[[job_id]] <- data.frame(
      job_id                 = job_id,
      label                  = sprintf("%s_%s_e%g_%s", MODEL, arm_token[[at]], ENRICHMENT,
                                       nref_token(nr)),
      model                  = MODEL,
      p_causal               = NA_real_,
      annotation_type        = at,
...
stopifnot(nrow(grid) == 10L, !anyDuplicated(grid$label))
...
cat(sprintf("  %d rows: LD {%s} x model {%s} x annotations {%s}\n",
            nrow(grid), paste(vapply(N_REF_LEVELS, nref_token, ""), collapse = ", "),
            MODEL, paste(ANNOTATION_TYPES, collapse = ", ")))
