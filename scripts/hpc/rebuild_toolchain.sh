...
#
# SELF-CONTAINED BY DESIGN. The venv's bin/python is a symlink to the EasyBuild
# interpreter this venv was created against, and that interpreter needs its
# module's library paths to start at all. The PBS job happens to load the module
# before invoking R, so the runner worked there - but from any shell without it
# (a login prompt, a cron job, a different submit script) every import failed
# with the module unloaded. Load it here so the runner behaves identically
# wherever it is called from, instead of silently inheriting its caller's env.
...
command -v module >/dev/null 2>&1 && module load $PYTHON_MODULE 2>/dev/null
source "$DEST/fmpy-venv/bin/activate"
# ABSOLUTE PATH, NOT \`exec python\`. Activating only prepends the venv to PATH,
# so if the venv's interpreter is broken or missing, \`python\` silently resolves
# to WHATEVER ELSE is on PATH - a system or conda interpreter. That fails
# confusingly when the other python lacks the packages, and far worse, SUCCEEDS
# when it has them, running every method against an interpreter nobody chose.
# Naming the interpreter outright makes a broken venv fail loudly.
exec "$DEST/fmpy-venv/bin/python" "\$@"
RUNNER
