...
#   OUT_DIR=$HOME/fine-mapping-benchmark/results/iter004/truth \
...
OUT_DIR="${OUT_DIR:-${PROJECT_ROOT}/results/iter004/truth}"
...
EXTRACT="${PROJECT_ROOT}/scripts/hpc/extract_truth.R"
[[ -f "$EXTRACT" ]] || { echo "ERROR: extract_truth.R missing" >&2; exit 1; }
...
JOB="$(mktemp -t fmbtruth_XXXXXX.sh)"
cat > "$JOB" <<EOF
#!/bin/bash
#PBS -N fmbtruth
#PBS -q ${QUEUE}
...
Rscript scripts/hpc/extract_truth.R "\${PBS_ARRAY_INDEX}" "${BENCH_ROOT}" "${OUT_DIR}"
...
When it finishes, check every row was rescued:
  ls ${OUT_DIR}/truth_*.rds | wc -l      # should be ${N_SIM}
